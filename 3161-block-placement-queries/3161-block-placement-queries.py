class Solution:
    ...
    def getResults(self, queries: List[List[int]]) -> List[bool]:
        ...
        for i in range(1, len(pos)):
            self.update(1,0,self.mx,pos[i],pos[i] - pos[i - 1])
        ...
        for i in range(len(queries) - 1, -1, -1):
            ...
            if queries[i][0] == 2:
                ...
                best = self.query(1,0,self.mx,0,prev_obstacle)
                ...
                self.update(1,0,self.mx,x,0)

                if idx + 1 < len(obstacles):
                    right_pos = obstacles[idx + 1]
                    self.update(1,0,self.mx,right_pos,right_pos - left_pos)
                obstacles.remove(x)
        return ans[::-1]
